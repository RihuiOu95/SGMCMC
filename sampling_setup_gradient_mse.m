function [clus_res,unif_res] = sampling_setup_gradient_mse(T, L, B, std)
    ...
    n_rep = 1000;
    emit_param_full = emit_param.eval_full_grad(y, trans_param, sgmcmc_param_clu, emit_prior);
    ground_truth = emit_param_full.mu_grad;
    % find the rare label;
    rare_label = find(abs(center - 20) == min(abs(center - 20)));
    unif_grad_error = zeros(n_rep, n_latent);
    clus_grad_error = zeros(n_rep, n_latent);
    for i = 1 : n_rep
        emit_param_clus = emit_param.eval_stoch_grad_IS(y, wt_alex_clus,  mb_size, trans_param, sgmcmc_param_clu, emit_prior);
        emit_param_unif = emit_param.eval_stoch_grad_IS(y, wt_unif,  mb_size, trans_param, sgmcmc_param_clu, emit_prior);
        clus_grad_error(i, :) = emit_param_clus.mu_grad ;
        unif_grad_error(i, :) = emit_param_unif.mu_grad; 
    end
    
    clus_grad_error = clus_grad_error - ground_truth;
    unif_grad_error = unif_grad_error - ground_truth;
    clus_res = sqrt(sum(clus_grad_error(:, rare_label).^2) / n_rep);
    unif_res = sqrt(sum(unif_grad_error(:, rare_label).^2) / n_rep);
end
