function [weights_alex, center, xisum] = weights_alex_clus(y, n_latent, N, L, eta) 
    ...
    for i = 1:n_latent 
        for j = 1:n_latent 
            for n = 1:N
                ...
                xi_idot(n,i) = sum(xi(n,i,:));
            end
        end
    end
    
    xisum = sum(xi,1) + 0.5;
    xi_idot_sum = sum(xi_idot,1) + 0.5;
    
    for n = 1:N 
        for i = 1:n_latent 
            weight_A(n,i) = sqrt(sum((xi(n,i,:) ./ xisum(1,i,:) -  xi_idot(n,i) ./ xi_idot_sum(i)).^2))+1e-10;
        end
    end
