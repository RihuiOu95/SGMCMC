function weights_alex = weights_alex_oracle(y, z, n_latent, N, L, eta) 
    ll = 2*L+1;
    ...
    for j = 1:n_latent 
        for n = 1:N 
           weights_mu(n,j) = c(n,j)*abs(ybar_chunk(n,j)-ybar(j)) + eta;
           weights_sigmasq(n,j) = c(n,j)*(s2(j)+s2_chunk(n,j)) + eta;
        end
    ...
    for i = 1:n_latent 
        for j = 1:n_latent 
            for n = 1:N
                idx = ((n-1)*ll+1):(n*ll);
                xi(n,i,j) = sum((z(idx(1:(clen-1)))==j).*(z(idx(2:clen))==i));
            end
        end
    end
    ...
    for n = 1:N 
        for i = 1:n_latent 
            weight_A(n,i) = sqrt(sum((xi(n,i,:).^2)./(xisum(1,i,:).^4))) + 1e-10;
        end
    end
    for i = 1:n_latent 
        weight_A(:,i) = weight_A(:,i)/sum(weight_A(:,i));
    end
